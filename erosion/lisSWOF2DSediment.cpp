            ...
            if (minTC < 0) {
                ...
                if (maxTC > 0 && CohesionSoil->Drc >= 0) {
                   ...
                   // TransportFactor = dt * TSettlingVelocitySS * SoilWidthDX->Drc * DX->Drc;
                    ...
                    detachment = Y->Drc * maxTC * TransportFactor * SoilWidthDX->Drc/wf;
...
void TWorld::SedimentDetachmentSSCont(double dt, cTMap *h, cTMap *w, cTMap *v,
                               cTMap *SS_, cTMap *SSC_, cTMap *SSTC_, cTMap *SSDet_, cTMap *Dep_, cTMap *SSVs_, int type)
{
    #pragma omp parallel for num_threads(userCores)
    FOR_ROW_COL_MV_L {
        ...
        if(h->Drc < HMIN) {
            ...
            //if (minTC < 0) {
                ...
                deposition  = qMax(TransportFactor, -SS);
                ...
                double conc  = MaxConcentration(sswatervol, SS+deposition);
                //detachment
                if (CohesionSoil->Drc >= 0) {
                   ...
                   // TransportFactor = dt * TSettlingVelocitySS * SoilWidthDX->Drc * DX->Drc;
                    ...
                    detachment = Y->Drc * qMax(SSTC_->Drc - conc, 0.0) * TransportFactor * SoilWidthDX->Drc/wf;
                    //check how much of the potential detachment can be detached from soil layer
                    //detachment = DetachMaterial(r,c,1, false, true, false, detachment);
            ...
            //}
            //### sediment balance
            SSDet_->Drc += qMax(deposition+detachment, 0.0);  // set to zero in mass balance
            Dep_->Drc += qMin(deposition+detachment, 0.0);
            SS += deposition + detachment;
            SS_->Drc = qMax(0.0,SS);
            SSC_->Drc = MaxConcentration(sswatervol, SS_->Drc);
