...
//---------------------------------------------------------------------------
void TWorld::TotalsHydro1D(void)
{
    ...
    FOR_ROW_COL_MV_L {
        InterceptionmmCum->Drc = vInterc[i_];
        if (SwitchIncludeET)
            InterceptionmmCum->Drc += vIntercETa[i_];
        if (SwitchHouses)
            InterceptionmmCum->Drc += vIntercHouse[i_];
        if(SwitchLitter)
            InterceptionmmCum->Drc += vLInterc[i_];
    ...
    if(InfilMethod != INFIL_NONE) {
       InfilTot += MapTotal(*InfilVol);// + MapTotal(*InfilVolKinWave);
        ...
        theta1tot = MapTotal1D(vThetaI1a)/nrCells;
        if (SwitchTwoLayer)
            theta2tot = MapTotal1D(vThetaI2a)/nrCells;
    }
