...
void GL3DDrawFunctions::DrawModelGLInstancedCubic(GL3DWidget * gl, GL3DModel * m, GL3DCamera * camera, GL3DSurface * surface,int rows_draw, int cols_draw, int rows_data, int cols_data,int layers, double increment, double incrementv, double rand_location, double rand_rotation, double rand_scale, float * rand, GL3DTexture * dens_1, GL3DTexture * dens_2, int upscale)
{

    for(int i = 0; i < m->GLVAO_List.length(); i++)
    {
        DrawModelGeometryWithMaterialGLInstancedCubic(gl,m->Geometry_List.at(i),m->m_Shader,m->GLVAO_List.at(i),m->Material_List.at(m->Material_Pointer.at(i)),camera, m->m_InstanceCount, surface,rows_draw,cols_draw,rows_data,cols_data,layers,increment,incrementv,rand_location,rand_rotation,rand_scale,rand,dens_1,dens_2, upscale);
...
void GL3DDrawFunctions::DrawModelGeometryWithMaterialGLInstancedCubic(GL3DWidget * gl, GL3DGeometry * g,GL3DShader * Shader, QOpenGLVertexArrayObject * vao,GL3DMaterial * mat, GL3DCamera * camera, int count, GL3DSurface * surface,int rows_draw, int cols_draw, int rows_data, int cols_data,int layers, double increment, double incrementv,double rand_location, double rand_rotation, double rand_scale, float * rand, GL3DTexture * dens_1, GL3DTexture * dens_2, int upscale)
{
   ...
   Shader->m_program->setUniformValue("upscale",(float) upscale);
   Shader->m_program->setUniformValue("spacing",(float) increment);
   Shader->m_program->setUniformValue("spacingv",(float) increment);
   ...
   Shader->m_program->setUniformValue("rows_data",(float(rows_data)));
   Shader->m_program->setUniformValue("cols_data",(float(cols_data)));
   Shader->m_program->setUniformValue("layers",(float(layers)));
   Shader->m_program->setUniformValue("rows_draw",(float(rows_draw)));
   Shader->m_program->setUniformValue("cols_draw",(float(cols_draw)));
