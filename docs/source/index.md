...
> 🚧 This documentation is an initial version and many sections still [need attention](about-openlisem/about.md#documentation-development)! We moved to Read the Docs in spring 2026
...
This documentation is designed as a guide for using OpenLISEM. For a first look see the [general introduction](using-openlisem/index.md) and the [quick start](./quick-start/setup-&-installation.md). In the [manual](./using-openlisem/manual/index.md) detailed information is provided to setup the input for a model run. The [theory](./theory/index.md) section explains the background and concepts used in OpenLISEM. Many topics in OpenLISEM are covered in three different sections, links between these topics are depicted with an icon: Input = [🔣](./using-openlisem/manual/Input/) , Data preparation = [🔨](./using-openlisem/manual/Prepare-data/Prepare-Data.md) and Theory = [📖](./theory/index.md). We aim to provide some tutorials to apply different functionalities of the model. OpenLISEM is an open source modelling project, and the model and wiki are under continuous [construction](./about-openlisem/about.md)!
