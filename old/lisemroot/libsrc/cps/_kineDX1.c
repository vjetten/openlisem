...
*/
        dtdx = deltaT/correctedDeltaX;
        ab = m_alpha[rowNr][colNr]*m_beta[rowNr][colNr];
        if (Sin+m_S0[rowNr][colNr] > 0)
        {
           powbeta=ab*powl(0.5*(Sin+m_S0[rowNr][colNr]),m_beta[rowNr][colNr]-1);
           m_S1[rowNr][colNr] = (dtdx*Sin+m_S0[rowNr][colNr]*powbeta)/(dtdx+powbeta);
        }
        else
           m_S1[rowNr][colNr]= 0;
...
{
	REAL4 **Q1Map, **S1Map;
	CONST_REAL4_MAP Q0Map,qMap,alphaMap, betaMap, S0Map,sMap, DXMap;
