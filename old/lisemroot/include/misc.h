...
void PauseOnTimer(void);
void PauseOffTimer(void);
long ReadTimer(void);
const char *StrTime(long t);
unsigned MilliSec(void);

/* filestat.c */
extern int DirectoryStat(const char *fileName);
...
extern int FilePathValid(const char *filePath);
...
/* mprint.c */
extern int MultiFprintf(FILE **streams, int nrStreams, const char *fmt, ...);
...
/* copyrigh.c */
#define CEES	    "C.G. Wesseling"
#define CEES_WILLEM "C.G. Wesseling & W.P.A. v. Deursen"
#define CEES_GERARD "C.G. Wesseling & G.B.M. Heuvelink"
#define CEES_STEVEN "C.G. Wesseling & S.M. de Jong"
extern void WriteTM(char *programmer, char *progName, char *version);

/* inssort.c */
extern void *InsertSorted(const void *key, void *base, size_t num, size_t width, QSORT_CMP cmp);
#ifdef LFIND_AVAILABLE
# include <search.h>
#else
 void *lfind(const void *key, const void *base, 
            unsigned int *num,  /* read only in effect */
            unsigned int width, int (*cmp)(const void *keyElement, const void *arrayElem));
#endif
 void *Lfind(const void *key, const void *base, 
            size_t num,  
            size_t width, int (*cmp)(const void *keyElement, const void *arrayElement));
...
/* dlutils.c */
extern void *DlSymbolLoad(void *handle, const char *extLib, const char *symbol);
extern BOOL DlLibraryLoad(void **h, const char *extLibNoExt);
extern const char *DlErrorMsg(void);
#ifdef BORLANDC
