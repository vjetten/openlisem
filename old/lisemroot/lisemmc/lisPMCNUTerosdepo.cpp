// *********************************************************
// ******** MULTICLASS SEDIMENT erosion and deposition *****
// ******** NUTRIENTS in suspension calculation ************
// *********************************************************
   ...
   //      mcalc(" DF# *= mif(WH gt 0, (1-exp(-SV#/(0.001*WH)*DTSEC))*WaterHVolin*RoadWidthFactor, 0)",6);
        ...
        //_spatial(REAL4, D50susp);
        //moved to paraminput for output
      ...
      //mcalc(" Dep# *= SV#*DTSEC*DX*FlowWidth",6);
        ...
        // ******************************************************
        // ************ calculate suspended nutrients ***********
        // ******************************************************

        if (SwitchNutrients)
        {
//VJ 030627 fixed typo xxxPdep = xxxPDep in all the code
            // deposition nut as fraction of total sediment if there is suspended matter
				calc("NutPDep   = mif(SedinMu0 gt 0.001 and NutPSuspension gt 0, NutPSuspension*Dep0/SedinMu0, 0) ");
				calc("NutNH4Dep = mif(SedinMu0 gt 0.001 and NutNH4Suspension gt 0, NutNH4Suspension*Dep0/SedinMu0, 0) ");
				calc("NutNO3Dep = mif(SedinMu0 gt 0.001 and NutNO3Suspension, NutNO3Suspension*Dep0/SedinMu0, 0) ");

            // erosion as uptake * local content * conversion
				calc("NutPDet   = DF0*NutPContent*NutPConversion ");
				calc("NutNH4Det = DF0*NutNH4Content*NutNH4Conversion ");
				calc("NutNO3Det = DF0*NutNO3Content*NutNO3Conversion ");

//VJ 030630 type error mixed suspended for nh4 and no3
            // balance of suspended nutrients
            calc(" NutPSuspension   = max(0, NutPSuspension + NutPDet + NutPDep) ");
            calc(" NutNH4Suspension = max(0, NutNH4Suspension + NutNH4Det + NutNH4Dep) ");
            calc(" NutNO3Suspension = max(0, NutNO3Suspension + NutNO3Det + NutNO3Dep) ");
				//VJ 030415 added separate maps for erosion and deposition
        }
