...
//        if(fraction * (cdx*K2DHOld->Drc*cdy) < K2DQ->Drc*dtr)
//        {
//            K2DQ->Drc =fraction* (cdx*K2DHOld->Drc*cdy)/dtr;
//        }
        K2DQ->Drc = std::min(K2DQ->Drc,fraction*cdx*cdy*K2DHOld->Drc/dtr );
        ...
        if(K2DQ->Drc > 0)
        {
            double mindtr = fraction * (cdx*K2DHOld->Drc*cdy)/K2DQ->Drc;
...
void TWorld::K2DSolvebyFlux(double dt)
{
    ...
    FOR_ROW_COL_MV
    {
        ...
        //                 xw = (K2DSlopeX->Drc/fabs(K2DSlopeX->Drc))*pow(fabs(K2DSlopeX->Drc),0.5)/pow(1.0 + pow(fabs(K2DSlopeY->Drc/K2DSlopeX->Drc),2.0),0.25);
        //                 yw = (K2DSlopeY->Drc/fabs(K2DSlopeY->Drc))*pow(fabs(K2DSlopeY->Drc),0.5)/pow(1.0 + pow(fabs(K2DSlopeX->Drc/K2DSlopeY->Drc),2.0),0.25);
    ...
    FOR_ROW_COL_MV
    {
        ...
        if(r != 0 && c != 0 && c != _nrCols-1 && r != _nrRows-1)
        {
            if(!pcr::isMV(K2DDEM->data[r+1][c]) && !pcr::isMV(K2DDEM->data[r-1][c]))
                Dhy = -0.5* (K2DDEM->data[r+1][c]-K2DDEM->data[r-1][c]);
            else
                Dhy = 0;

            if(!pcr::isMV(K2DDEM->data[r][c+1]) && !pcr::isMV(K2DDEM->data[r][c-1]))
                Dhx = -0.5* (K2DDEM->data[r][c+1]-K2DDEM->data[r][c-1]);
            else
                Dhx = 0;
        ...
        {
            K2DSlope->Drc = 0.01;
            //qDebug() << Dhx << Dhy << r << c <<"K2DSlope NAN";
        }
